...
| 0014_add_multicurrency_fx.sql | Applied — adds `transactions.fx_rate_to_base` (numeric, nullable). **Unused**: the per-transaction FX approach this was built for was tried and reverted (see DECISIONS.md D119-D131) in favor of converting only at portfolio-total display time using today's rate — kept rather than dropped (D128), nothing reads/writes it. `portfolios.base_currency` already existed since 0001 and needed no change |
